...
Use this checklist before publishing v1.2.0 release collateral.
...
- Run verification: `PYTHONPATH=src python -m pytest`, `PYTHONPATH=src python -m earnings_event_playbook selfcheck`, `PYTHONPATH=src python -m earnings_event_playbook review-packet --out demo/review-packet`, `PYTHONPATH=src python -m earnings_event_playbook coldstart-audit --manifest demo/review-packet/review-packet-manifest.json --out demo/coldstart-audit.md --json-out demo/coldstart-audit.json`, and `UV_CACHE_DIR=/tmp/uv-cache uv build --no-build-isolation`.
...
- Confirm `release_manifest.json`, `CHANGELOG.md`, `pyproject.toml`, `src/earnings_event_playbook/__init__.py`, and `skills/agent/earnings-event-playbook/SKILL.md` all name v1.2.0.
