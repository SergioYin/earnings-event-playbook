...
- Human-readable demo output is checked in at `demo/playbook.md`, `demo/post-event-compare.md`, and `demo/visual-receipt.md`; deterministic machine output is checked in at `demo/playbook.json`, `demo/post-event-compare.json`, and `demo/visual-receipt.json`.
...
- Demo outputs: `demo/events.json`, `demo/portfolio.json`, `demo/actuals.json`, `demo/playbook.md`, `demo/playbook.json`, `demo/post-event-compare.md`, `demo/post-event-compare.json`, `demo/index.html`, `demo/visual-receipt.md`, `demo/visual-receipt.json`
...
Latest local run on 2026-07-08: pytest passed with 19 tests, unittest discovery passed, demo regeneration completed, documented post-event compare completed, visual receipt regeneration completed, selfcheck scanned 37 public files, and `UV_CACHE_DIR=/tmp/uv-cache uv build --no-build-isolation` produced both 0.3.0 sdist and wheel artifacts.
...
- Visual receipts record local demo artifact roles, sizes, and SHA-256 hashes; they are release evidence, not an audit or compliance system.
